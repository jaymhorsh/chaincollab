...
export function Clip({ className }: { className?: string }) {
  ...
  const createClipComposed = useCallback(async (opts: ClipPayload) => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error('Failed to create a clip. Please try again in a few seconds.');
      }
    });
  }, []);
  ...
  return (
    <Player.LiveIndicator className={className} asChild>
      ...
      >
        {isPending ? <LoadingIcon className="h-full w-full animate-spin" /> : <ClipIcon className="w-full h-full" />}
      </Player.ClipTrigger>
