...
const clipPayloadSchema = z
  .object({
    ...
    startTime: z.number().refine(isValidUnixTimestamp, {
      message: 'Start time must be a valid Unix timestamp in milliseconds and within the past hour.',
    }),
    endTime: z.number().refine(isValidUnixTimestamp, {
      message: 'End time must be a valid Unix timestamp in milliseconds and within the past hour.',
    }),
  })
  .refine(
    (data) => {
      return data.endTime > data.startTime && data.endTime <= data.startTime + 60000;
    },
    {
