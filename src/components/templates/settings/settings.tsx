...
const Settings: React.FC = () => {
  const {
    ...
    linkWallet, linkEmail, linkApple, linkDiscord , linkGithub, linkGoogle, linkPhone, linkTwitter
  } = usePrivy();
     ...
     <div className="mt-4 py-6 w-full flex-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {accounts.map((account, idx) => (
          <div
            key={idx}
            className="flex flex-col min-h-30 justify-between border p-4 rounded-lg shadow-sm"
          >
            <p className="font-medium">{account.name}</p>
              ...
              onClick={() => {
                if (account.value) {
                  if (account.type === 'number') {
                    handleUnlink(
                      account.unlink as (account: number) => Promise<any>,
                      account.value as number
                    );
                  } else {
                    handleUnlink(
                      account.unlink as (account: string) => Promise<any>,
